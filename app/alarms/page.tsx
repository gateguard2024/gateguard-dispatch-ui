...
// ============================================================================
// STABLE HLS PLAYER (Direct Cluster Playback)
// ============================================================================
const SmartVideoPlayer = ({ camId, token, type = 'main', offsetSeconds = 0 }: any) => {
    ...
    useEffect(() => {
        ...
        // 1. Build Direct URL with Auth 'A' Parameter
        // This bypasses the Vercel Handshake to fix the "undefined" segment errors
      ...
      <div className="flex justify-between items-center mb-4 bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-md shadow-2xl">
        <div className="flex items-center gap-6">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all border ${isPatrol ? 'bg-amber-500 border-amber-500 text-black animate-pulse shadow-[0_0_10px_rgba(245,158,11,0.4)]' : 'bg-white/5 border-white/10 text-slate-400'}`}
            >
      ...
      <div className="flex flex-1 gap-4 overflow-hidden">
          ...
          <div className="space-y-2 overflow-y-auto custom-scrollbar flex-1 pr-1">
            {MARBELLA_CAMERAS.map((cam) => (
              <div 
                ...
                className={`p-4 rounded-2xl cursor-pointer transition-all border ${activeCam?.id === cam.id ? 'bg-blue-600/20 border-blue-500/50 shadow-inner' : 'bg-white/5 border-transparent hover:bg-white/10'}`}
              >
                <span className="text-sm font-bold block truncate">{cam.name}</span>
                <span className="text-[8px] text-slate-500 font-mono mt-1 block">ID: {cam.id}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {view === 'incident' && activeCam && (
            <div className="absolute inset-0 flex">
              <div className="flex-1 border-r border-white/10 relative">
                ...
                <span className="absolute bottom-6 left-6 bg-amber-600 px-3 py-1 rounded-lg text-[9px] font-black shadow-xl uppercase tracking-widest">Incident Replay</span>
              </div>
              <div className="flex-1 relative">
                <SmartVideoPlayer camId={activeCam.id} token={activeToken} offsetSeconds={0} />
                <span className="absolute bottom-6 left-6 bg-red-600 px-3 py-1 rounded-lg text-[9px] font-black shadow-xl uppercase tracking-widest">Live Status</span>
              </div>
            </div>
          )}

          {view === 'map' && <div className="absolute inset-0 flex items-center justify-center text-slate-800 text-[10px] font-black tracking-[1em] uppercase bg-slate-950">Site Map Module Pending</div>}
        </div>
        ...
        <div className="w-72 shrink-0 flex flex-col gap-4">
          ...
          <div className="bg-gradient-to-br from-indigo-900/20 to-transparent border border-indigo-500/20 rounded-[2rem] p-6 flex-1 shadow-2xl flex flex-col">
            <h3 className="text-[10px] font-black text-indigo-400 uppercase mb-6 tracking-widest text-center border-b border-white/5 pb-2">SOP Checklist</h3>
            ...
            <button className="w-full bg-red-600 hover:bg-red-500 py-4 rounded-2xl text-[10px] font-black transition-all mt-10 shadow-lg shadow-red-900/40">FLAG TO DATABASE</button>
          </div>
        </div>
      </div>
    </div>
