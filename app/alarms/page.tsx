...
// ============================================================================
// STABLE HLS PLAYER (Direct Cluster via 'A' Parameter)
// ============================================================================
const SmartVideoPlayer = ({ camId, token, type = 'main', offsetSeconds = 0, controls = false }: any) => {
    ...
    useEffect(() => {
        ...
        // Construct the Direct Cluster URL with the 'A' (Authorization) parameter
        // This is the V3 standard for stable media playback
    ...
    <div className="w-full h-full flex flex-col p-4 bg-[#020408] text-white overflow-hidden font-sans">
          ...
          <div className="flex bg-black/40 border border-white/10 rounded-xl p-1 shadow-inner">
            {['grid', 'live', 'incident', 'map'].map(v => (
              <button 
                ...
                className={`px-6 py-2 rounded-lg text-[10px] font-black transition-all ${view === v ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}
              >
                {v.toUpperCase()}
              </button>
            ))}
      ...
      <div className="flex flex-1 gap-4 overflow-hidden">
        ...
        <div className="flex-1 bg-black border border-white/10 rounded-[2.5rem] relative overflow-hidden shadow-2xl flex flex-col">
          ...
          {view === 'map' && <div className="absolute inset-0 flex items-center justify-center text-slate-800 text-[10px] font-black tracking-[1em] uppercase bg-slate-950">Map Module Loading...</div>}
        </div>
        ...
        <div className="w-72 shrink-0 flex flex-col gap-4">
          ...
          <div className="bg-gradient-to-br from-indigo-900/20 to-transparent border border-indigo-500/20 rounded-[2rem] p-6 flex-1 shadow-2xl flex flex-col">
            ...
            <button className="w-full bg-red-600 hover:bg-red-500 py-4 rounded-2xl text-[10px] font-black transition-all mt-10 shadow-lg shadow-red-900/40 uppercase">Flag Incident</button>
          </div>
        </div>
      </div>
    </div>
